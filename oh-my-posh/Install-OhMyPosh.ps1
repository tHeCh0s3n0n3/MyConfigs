...
'@)
    ...
    Add-Content -Path $PROFILE -Value "@(&""(Get-Command oh-my-posh.exe).Path"" init pwsh -c ""$HOME\AppData\Local\Programs\oh-my-posh\themes\tarekf.omp.yaml"" --print) -Join ""`n"" | Invoke-Expression"
    Write-Host " Done"
}